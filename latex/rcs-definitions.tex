...
\begin{tabular}{@{}cp{6.5cm}c@{}}
  ...
  \midrule
  $\decay_3$ & Nominal decay --- policy crystallization rate
    ${\sim}\,1/100$ days & $0.010$ \\
  $L_{\param,3} \rho_3$ & Setpoint adjustment rate
    ${\leq}\,1/\text{month}$ & $0.001$ \\
  $L_{d,3} \eta_3$ & Structural changes ${\sim}\,1/\text{quarter}$
    (new primitives, gates) & $0.0005$ \\
  $\beta_3 \taubar_3$ & Human review delay (hours--days for
    policy changes) & $0.002$ \\
  $\frac{\ln \nu_3}{\taua[3]}$ & Profile switches (extremely rare)
    & $0.0001$ \\
  \midrule
...
($\stab_3 = 0.0064$ vs.\ $\stab_0 = 1.455$, $\stab_1 = 0.411$,
$\stab_2 = 0.069$).  This is expected from the time-scale separation:
governance operates on the slowest time scale (days--weeks), so $\decay_3$ is
smallest.  The system is most fragile at its highest level --- rapid governance
changes would violate stability.  This matches the design intent: policy changes
