...
populates the 7-tuple on its own state space; the final column names
the published witness that produces the per-level decay
rate~$\decay_i > 0$ required by Assumption~\ref{ass:decay}.  With
(H1)--(H7) discharged by the witnesses above,
Theorem~\ref{thm:recursive} then composes levels into a finite-depth
